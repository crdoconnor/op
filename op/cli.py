import os, whichfiles, yaml_config, default_editor, op_command, parent_checker
...
def cli():
    """CLI interpreter for op."""
    args = argv[1:]
    config = yaml_config.OpConfig()
    cwd = os.getcwd()
    ...
    if len(args) == 0 or len(args) == 1 and args[0] in ['-h', '--help', 'help']:
        stdout.write("Usage:\n\n")
        stdout.write("                   op keyword1 - Open file matching keyword1.\n")
        stdout.write("          op keyword1 keyword2 - Open file matching keyword1 and keyword2.\n")
        stdout.write(" op keyword1 keyword2 keyword3 - Open file matching all keywords.\n")
        stdout.write("                       op last - Open last 10 files that you modified.\n")
        stdout.write("                     op last 5 - Open last 5 files that you modified.\n")
        exit(1)

    if config.no_config():
        commands = [op_command.OpCommand({'name':'default', 'command': default_editor.default_editor()}),]
    else:
    ...
    # First check to see if there are files specified directly in the args (e.g. if user wnent to a dir and went op *)
    ...
    # Last argument is a number? Use for "op last 5"
    if args[-1].isdigit():
